...
There are 4 included SSDT-NVMe-ANSx-X files which rename ANSI devices and inject information into the System Information/PCI section. The renaming behavior forces the disk icons to updated and changed from external to internal icons on the desktop. The information changes in these files should be changed to reflect the brand and sizes of NVMe drives that you are using in your build. However, as these ANSI files are not essential for booting, all may be ignored if you are not concerned about desktop icons. (Two of the NVMe files, _ANS3-GPP0-ANS3_ and _ANS3-GPP2_, are described below in section A6.)
